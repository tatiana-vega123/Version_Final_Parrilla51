...
def actualizar_estado(id_pedido):
    ...
    estados_validos = ['pendiente', 'en preparacion', 'entregado', 'cancelado']
...
# fechas RESERVAS
# ===============================
@empleado_bp.route("/verificar_fecha_reserva", methods=["POST"])
def verificar_fecha_reserva():
    print("🔵 LLEGA AL ENDPOINT /verificar_fecha_reserva")

    data = request.json
    fecha = data.get("fecha")
    id_reserva_editando = data.get("id_reserva")

    cur = mysql.connection.cursor(MySQLdb.cursors.DictCursor)

    # Consulta para verificar si existe una reserva en esa fecha
