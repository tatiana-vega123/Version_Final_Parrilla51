...
@cliente_bp.route("/reservas/fechas-bloqueadas")
def fechas_bloqueadas():
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute("SELECT fecha FROM reservas")
    fechas = cursor.fetchall()
    cursor.close()

    # Convertir fechas datetime.date a "YYYY-MM-DD"
    fechas_bloqueadas = [
...
def hacer_pedido():
    ...
    # =====================================================
    # 🔥 NUEVO FORMATO: acompañamientos por PLATO y PORCIÓN
    # =====================================================
    acomp_por_plato = {}  # { id_producto : [lista de acompañamientos en orden] }

    for item in carrito:

        cantidad = item["cantidad"]
        id_prod = item["id_producto"]
        acomp_por_plato[id_prod] = []

        # Leer acompañamientos por cada porción
        for i in range(cantidad):
            key = f"acompanamientos_{id_prod}_{i}"
            acomp_seleccionados = request.form.getlist(key)

            if len(acomp_seleccionados) != 2:
                flash(
                    f"⚠️ Cada porción de '{item['nombre']}' debe tener 2 acompañamientos. "
                    f"Porción {i+1} tiene {len(acomp_seleccionados)} seleccionados.",
                    "warning"
                )
                return redirect(url_for("cliente.cliente_carrito"))

            # Guardar acompañamientos para esta porción
            acomp_por_plato[id_prod].extend(acomp_seleccionados)
    ...
    try:
        ...
        for item in carrito:
            cur.execute("SELECT nombre, cantidad, estado FROM productos WHERE id_producto = %s", 
                        (item["id_producto"],))
            producto = cur.fetchone()
            ...
            if not producto:
                ...
                stock_insuficiente.append(
                    f"{item['nombre']} (solo quedan {producto['cantidad']} unidades)"
                )

        # Acompañamientos
        for acomp_list in acomp_por_plato.values():
            for id_acomp in acomp_list:
                cur.execute("SELECT nombre, cantidad, estado FROM productos WHERE id_producto = %s",
                            (int(id_acomp),))
                acomp = cur.fetchone()
        ...
        cur.execute("""
            ...
            VALUES (%s, CURDATE(), CURTIME(), %s, 'pendiente',
                    %s, %s, %s, %s)
        ...
        for item in carrito:
            ...
            """, (id_pedido, id_prod, cantidad, item["precio"]))

            # Restar stock del plato
            cur.execute("""
                UPDATE productos 
            ...
            # 🔥 Guardar acompañamientos por porción
            for id_acomp in acomp_por_plato[id_prod]:
                cur.execute("""
                    INSERT INTO detalle_pedido (
                        cod_pedido, cod_producto, cantidad, precio_unitario
                    ) VALUES (%s, %s, 1, 0)
        ...
        flash(f"✅ Pedido #{id_pedido} confirmado. Total: ${total:,.0f}", "success")
        return redirect(url_for("dashboard.cliente_dashboard"))
