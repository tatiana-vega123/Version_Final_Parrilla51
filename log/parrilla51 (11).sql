...
CREATE TABLE `entradas_inventario` (
  ...
  `tipo_entrada` enum('compra','devolucion','ajuste','donacion') DEFAULT 'compra'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
INSERT INTO `entradas_inventario` (`id_entrada`, `cod_producto`, `cantidad`, `precio_compra`, `id_proveedor`, `numero_lote`, `fecha_entrada`, `fecha_vencimiento`, `id_usuario`, `observaciones`, `tipo_entrada`) VALUES
(1, 40, 12, 2800.00, 2, '2025-001', '2025-11-13 22:04:19', '2025-12-21', 44, '', 'compra'),
(2, 43, 20, 5600.00, 2, '2025-001', '2025-11-13 22:08:48', '2025-12-21', 44, '', 'compra');

--
-- Disparadores `entradas_inventario`
--
DELIMITER $$
CREATE TRIGGER `trg_actualizar_stock_entrada` AFTER INSERT ON `entradas_inventario` FOR EACH ROW BEGIN
    ...
    VALUES (
        CONCAT('? Entrada: +', NEW.cantidad, ' unidades de ', 
               (SELECT nombre FROM productos WHERE id_producto = NEW.cod_producto)),
        NOW(),
        'stock'
    );
END
...
(13, 20, '2025-11-13', '20:35:31', 100000.00);
...
(2, 'Churrasco pequeño (230g)', 26, 'Corte de res a la parrilla tamaño pequeño', 25000, NULL, NULL, 1, 'churrasco_pequeno.png', 'Disponible'),
...
(43, 'Aguacate', 30, 'Porción de aguacate fresco', 7000, NULL, NULL, 10, 'aguacate.jpg', 'Disponible'),
...
INSERT INTO `proveedores` (`id_proveedor`, `nombre`, `contacto`, `telefono`, `correo`, `direccion`, `estado`, `fecha_registro`) VALUES
(1, 'Carnes Selectas S.A.', 'Juan Pérez', '3001234567', 'ventas@carnesselectas.com', 'Calle 45 #23-12, Bogotá', 'activo', '2025-11-13 21:45:54'),
(2, 'Distribuidora de Alimentos', 'María González', '3109876543', 'contacto@distalimentos.com', 'Carrera 30 #15-45, Bogotá', 'activo', '2025-11-13 21:45:54'),
(3, 'Verduras Frescas Ltda', 'Pedro Ramírez', '3156789012', 'info@verdurasfrescas.com', 'Avenida 68 #100-23, Bogotá', 'activo', '2025-11-13 21:45:54');
...
CREATE TABLE `salida` (
  ...
  `cantidad` bigint(20) DEFAULT NULL,
  `tipo_salida` enum('venta','merma','devolucion','ajuste') NOT NULL DEFAULT 'venta',
  `motivo_merma` enum('vencido','dañado','desperdicio','otro') DEFAULT NULL,
...
--
DELIMITER $$
CREATE TRIGGER `trg_actualizar_stock_salida` AFTER INSERT ON `salida` FOR EACH ROW BEGIN
    ...
    -- Registrar alerta según el tipo de salida
    IF NEW.tipo_salida = 'merma' THEN
        -- Alerta especial para mermas
        INSERT INTO alertas(mensaje, fecha, tipo)
        VALUES (
            CONCAT('⚠️ Merma: -', NEW.cantidad, ' unidades de ', nombre_producto,
                   ' | Motivo: ', IFNULL(NEW.motivo_merma, 'No especificado')),
            NOW(),
            'stock'
        );
    ELSEIF NEW.tipo_salida = 'venta' THEN
        -- Alerta para ventas (opcional, puedes comentar si no quieres alertas de ventas)
        INSERT INTO alertas(mensaje, fecha, tipo)
        VALUES (
            CONCAT('? Venta: -', NEW.cantidad, ' unidades de ', nombre_producto),
            NOW(),
            'stock'
        );
    ELSE
        -- Alerta para otros tipos de salida
        INSERT INTO alertas(mensaje, fecha, tipo)
        VALUES (
            CONCAT('? Salida (', NEW.tipo_salida, '): -', NEW.cantidad, ' unidades de ', nombre_producto),
            NOW(),
            'stock'
        );
    END IF;
...
--
ALTER TABLE `detalle_pedido_restaurante`
  MODIFY `id_detalle_pedido_restaurante` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11;
...
--
ALTER TABLE `pagos_restaurante`
  MODIFY `id_pago_restaurante` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=14;
...
--
ALTER TABLE `entradas_inventario`
  ADD CONSTRAINT `entradas_inventario_ibfk_1` FOREIGN KEY (`cod_producto`) REFERENCES `productos` (`id_producto`) ON DELETE CASCADE,
  ADD CONSTRAINT `entradas_inventario_ibfk_2` FOREIGN KEY (`id_proveedor`) REFERENCES `proveedores` (`id_proveedor`) ON DELETE SET NULL,
